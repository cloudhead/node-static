...
Server.prototype.parseByteRange = function(req) {

    var byteRange = {from:0, to:0, valid: false};
...
Server.prototype.respondNoGzip = function (pathname, status, contentType, _headers, files, stat, req, res, finish) {
    var mtime           = Date.parse(stat.mtime),
        ...
        startByte       = 0,
        length          = stat.size,
        byteRange       = this.parseByteRange(req);

    /* Handle byte ranges */
    if(files.length == 1 && byteRange.valid && byteRange.to < length) {
...
Server.prototype.stream = function (pathname, files, buffer, startByte, res, callback) {
    ...
    //console.log("Buffer length: ", buffer.length, " start byte: ", startByte);
