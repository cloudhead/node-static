...
this.Server.prototype.serveLess = function (pathname, status, headers, req, res, finish) {
    ...
    fs.stat(pathname, function (e, stat) {
        ...
        fs.readFile(pathname, 'utf8', function (e, data) {
            ...
            require('less').render(data, {compress: true}, function (e, css) {
                if (e) {
                    return promise.emit('error', e);
                }

                var newPath = path.join(path.dirname(pathname), 
                                        '__'+path.basename(pathname));

                fs.writeFile(newPath, css, function (e) {
                    if (e) {
                        return promise.emit('error', e);
                    }
                    ...
                    fs.stat(newPath, function (e, stat) {
                        ...
                        that.respond(null, status, headers, [newPath], stat, req, res, finish);
                    });
                });
            });
        });
