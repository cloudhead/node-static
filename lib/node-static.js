...
this.Server = function (root, options) {
    ...
    if (this.cache in this.options) {
        function setCacheOption (option, type, def) {
          that[option] = typeof(that.options[option]) === type ? that.options[option] : def;
        }

        setCacheOption('cache', 'number', false);
        setCacheOption('revalidate', 'boolean', false);
        setCacheOption('public', 'boolean', false);
        setCacheOption('private', 'boolean', false);
    }

    if (this.cache !== false) {
        function setCacheControl(expr, value) {
          if (expr) cacheControl.push(value);
        }
        
        var cacheControl = [];
        setCacheControl(this.public === true && !this.private, 'public');
        setCacheControl(this.private === true && !this.public, 'private');
        setCacheControl(true, 'max-age=' + this.cache);
        setCacheControl(this.revalidate === true, 'must-revalidate');
        
        this.defaultHeaders['Cache-Control'] = cacheControl.join(', ');
    }
...
this.Server.prototype.stream = function (pathname, files, buffer, res, callback) {
    (function streamFile(files, offset) {
        ...
        if (file) {
            ...
            var target = file[0] === '/' ? file : path.join(pathname || '.', file); 
